...
import br.com.wells.core.domain.usuario.model.SenhaCriptografada;
...
public final class AlterarSenhaUsuarioUseCaseImpl implements AlterarSenhaUsuarioUseCase {
	...
	public AlterarSenhaUsuarioUseCaseImpl(AlterarSenhaUsuarioGateway alterarSenhaUsuarioGateway,
	...
	@Override
	public void execute(final Long id, final String senhaAtual, final String novaSenha, final String confirmaSenha) {
		validarCamposObrigatorios(id, senhaAtual, novaSenha, confirmaSenha);

		validarConfirmarSenha(novaSenha, confirmaSenha);

		var usuarioExistente = consutlarUsuarioPorIdUseCase.find(id);
		if (!SenhaCriptografada.isSenhaIguais(senhaAtual, usuarioExistente.senha())) {
		...
		var usuario = usuarioExistente.alterar(novaSenha);

		alterarSenhaUsuarioGateway.execute(usuario);
	}

	private static void validarConfirmarSenha(String novaSenha, String confirmaSenha) {
		if (!novaSenha.equals(confirmaSenha)) {
			throw new SenhaInvalidaException("Nova senha não confere com confirmação de senha.");
		}
	}

	private void validarCamposObrigatorios(final Long id, final String senhaAtual, final String novaSenha,
	...
	private void validarNaoVazio(final String campo, final String nomeCampo) {
		if (campo == null || campo.trim().isEmpty()) {
			throw new SenhaInvalidaException(String
